...
def parse_sensor_list_file(file_name: str) -> list[str]:
    ...
    try: 
        ...
        raise ValueError(f"File {file_name} not found. Please use a valid .txt file.") from e
    
    if len(sensor_list) == 0:
        raise ValueError("No sensor IDs found in the file. Please provide a valid list of sensor IDs.")
...
def fetch_aqi(sensor_list: list[str], api_key) -> aqi.EPA_AQI:
    ...
    average_pm25 = statistics.mean([sensor['sensor']['stats']['pm2.5_60minute'] for sensor in raw_data])
